...
// Single place where the verdict enum and source rules are enforced, for both
// the video path and the selection path.
export function shapeVerdict(item, result) {
...
export async function verifyClaim(item) {
  ...
  return shapeVerdict(item, result);
}
